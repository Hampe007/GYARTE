...
public class TerrainPrefabPainter : EditorWindow
{
    ...
    void DrawPrefabRuleSection()
    {
        ...
        for (int i = 0; i < prefabRules.Length; i++)
        {
            ...
            if (!ruleFoldouts[i])
            {
                // Close the vertical because we opened it above
                EditorGUILayout.EndVertical();
            ...
            EditorGUILayout.EndVertical(); // <-- ends inner box
            EditorGUILayout.EndVertical(); // <-- ends outer box
            EditorGUILayout.Space(4);
        }
    ...
    void PaintPrefabs(TerrainData td, float[,] heights, float[,,] alpha)
    {
        ...
        void ProcessSample(float nx, float ny, float wx, float wz)
        {
            ...
            for (int i = 0; i < prefabRules.Length; i++)
            {
                ...
                {
                    // variant selected → place under variant folder
                ...
                {
                    // no variants → place directly under rule
    ...
    void DeleteAllSpawnedPrefabs()
    {
        ...
        if (terrain && terrain.terrainData)
        {
            ...
            {
                if (rule.clearedGrass == null) continue;
